...
var nodeStore = Reflux.createStore({
  ...
  loadNodes(ids) {
    ...
    xhr(
      { uri: `${API06}nodes/?nodes=${ids.join(",")}`, method: "GET" },
      (err, resp, body) => {
        if (err) return console.error(err);
    ...
    return (
      <div className="pad2">
        <div className="pad1 space-bottom1">
          COFFEEDEX is built on OpenStreetMap and requires an OpenStreetMap
          account.
        </div>
        <button
          onClick={userLogin}
...
var StaticMap = React.createClass({
  render() {
    return (
      ...
      <img
        src={
          `https://api.tiles.mapbox.com/v4/${MAP}/` +
          `${PIN}(${this.props.location.longitude},${this.props.location.latitude}),` +
          (this.props.self
            ? `${LOC}(${this.props.self.longitude},${this.props.self.latitude})`
            : "") +
  ...
  render() {
    return (
      <div>
        ...
        {this.state.user ? (
          <div className="pad2">
            {!values(this.state.nodes).length && (
              <div className="pad4 center">Loading...</div>
            )}
        ...
        <div className="center dark space-bottom1">
          <div className="pill space-top1">
            <Link className="button stroke quiet icon globe" to="world_map">
              World Map
            </Link>
            <Link className="button stroke quiet" to="help">
              Help
            </Link>
          </div>
        </div>
...
var Success = React.createClass({
  ...
  render() {
    return (
      <Link to="list" className="col12 center pad4">
        <h2>
          <span className="big icon check"></span> Saved!
        </h2>
      </Link>
    );
  }
...
var Help = React.createClass({
  ...
  render() {
    return (
      <div>
        <Link to="list" className="home icon button fill-darken2 col12">
          home
        </Link>
        <div className="pad1y">
          <div className="round fill-lighten0 pad2 dark">
            <p>
              <strong>COFFEEDEX</strong> is a community project that aims to
              track the price of house coffee everywhere.
            </p>
            <p>
              The data is stored in <a href="http://osm.org/">OpenStreetMap</a>,
              a free and open source map of the world, as tags on existing
              coffeehops. There are 150,000+.
            </p>
            <p>
              Maps in this application are &copy;{" "}
              <a href="http://mapbox.com/">Mapbox</a>.
            </p>
            <p>
              COFFEEDEX data stored in OpenStreetMap is{" "}
              <a href="http://www.openstreetmap.org/copyright">
                available under the ODbL license.
              </a>
            </p>
            <p>
              This is also an open source project. You can view the source code,
              clone it, fork it, and make new things with it as inspiration or
              raw parts.
            </p>
            <a
            ...
            <p>
              <span className="icon mobile"></span> COFFEEDEX also works great
              on phones! Try it on your phone and add it to your iPhone home
              screen - it'll look even prettier.
            </p>
            <h2>FAQ</h2>
            <ul>
              <li>
                <strong>Which coffee?</strong> This site tracks the price of{" "}
                <em>house coffee</em> for here. In many cases, that means a 12oz
                drip, but if all coffees are pour-overs or your country uses
                different standard size, the overriding rule is cheapest-here.
              </li>
            </ul>
          </div>
        </div>
  ...
  render() {
    return (
      <div>
        ...
        {this.state.nodes && (
          <div className="pin-bottom fill-navy pad0 center">
...
var Editor = React.createClass({
  ...
  render() {
    var node = this.state.node;
    /* jshint ignore:start */
    if (!node) return <div className="pad4 center">Loading...</div>;
    return (
      <div className="col12">
        <Link to="list" className="home icon button fill-darken0 unround col12">
          home
        </Link>
        <StaticMap location={node.location} self={this.state.location} />
        <div className="pad1 col12 clearfix">
          <div className="col12">
            <div className="center">how much for a cup of joe at</div>
            <h1 className="center">{node.tags.name}</h1>
          </div>
